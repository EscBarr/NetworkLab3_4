...
{
    public class SocketPortScaner
    {
        public SocketPortScaner()
        {
            List<PortInfo> pi = SocketPortScaner.GetOpenPort();
            foreach (var item in pi)
            {
                Console.WriteLine(item.PortNumber + " " + item.Local + " " + item.Remote + " " + item.State);
            }
        }
